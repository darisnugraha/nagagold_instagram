            ...
            <div class="p-5" id="input">
                <div class="mt-3">
                    <div>
                        ...
                        <input type="text" disabled value="<?= $row->kode_barcode ?>" class="input w-full border mt-2"
                ...
                <div class="mt-3">
                    <div>
                        ...
                        <input type="text" disabled value="<?= $row->nama_barang ?>" class="input w-full border mt-2"
                            placeholder="Kode Barang">
                    </div>
                </div>
                <div class="mt-3">
                ...
                <input type="hidden" value="<?= $row->kode_kategori ?>" class="kode_grp">
                <input type="hidden" value="<?= $row->kode_kelompok ?>" class="kode_klpk">
                <div class="mt-3">
                    <div>
                ...
                <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
                    ...
                    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
                        <div class="text-center">
                            <a href="javascript:;" class="addgambar button inline-block bg-theme-1 text-white">Tambah
                                Gambar</a>
                        </div>
                    </div>
                </div>
                            ...
                            <div>
                                <div class="image-upload">
                                    <label for="file-input1" class="pointer">
                                        <img id="<?= $data_gambar[$i]->kode_gambar ?>" width="200"
                                            onError="this.onerror=null;this.src='<?php echo base_url('assets/images/notfound.png') ?>';"
                                            src="<?= $data_gambar[$i]->lokasi_gambar ?>" alt="image preview" />
                                    </label>
                                    <input value="<?= $data_gambar[$i]->lokasi_gambar ?>" name="nama_file[]"
                                        type="hidden" />
                                    <div class="relative mt-2">
                                        <input id="file-input1" class="input pr-16 w-full border col-span-4"
                                            name="photo<?= $i ?>"
                                            onchange="document.getElementById('<?= $data_gambar[$i]->kode_gambar ?>').src = window.URL.createObjectURL(this.files[0])"
                                            type="file" />
                                        <div
                                            class="absolute top-0 right-0 rounded-r w-16 h-full flex items-center justify-center bg-gray-100 border text-gray-600">
                                            File</div>
                                    </div>
                                    <button type="button" onclick="hapusgambar('<?= $data_gambar[$i]->kode_gambar ?>');"
                                        class="button bg-theme-1 input w-full btn-block text-white">Hapus</button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        complete: function(respons) {
            ...
            if (feedback.status == "berhasil") {
                $.each(feedback.data, function(index, element) {
                    let selected = "";
                    if (idku == element.kode_kategori) {
                        selected = "selected";
                    }
                    $('.sub_jenis').html('');
                    $('.sub_jenis').append(`
                                <option  ` + selected + ` value="` + element.kode_jenis + `"> ` + element.nama_jenis + ` </option>
                        `);
                })
            } else {
                $('.sub_jenis').html('');
            ...
            if (feedback.status == "berhasil") {
                if (feedback.data.length === 0) {
                    ...
                    $.each(feedback.data, function(index, element) {
                        ...
                        $('.id_jenis').html('');
                        $('.id_jenis').append(`
                                    <option ` + selected + ` value="` + element.kode_jenis + `"> ` + element
                            .nama_jenis + ` </option>
                            `);
                    })
                }
