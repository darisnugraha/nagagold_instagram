<?php foreach ($DataRekening->data as $row) : ?>
    <div class="p-5" id="header-footer-modal">
        <div class="preview">
            <div class="modal" id="editnorek<?= $row->no_rek  ?>">
                <div class="modal__content">
                    ...
                    <form action="<?= base_url('edit-norek') ?>" method="POST">
                        <div class="p-5 grid grid-cols-12 gap-4 row-gap-3">
                            <div class="col-span-12 sm:col-span-12">
                                <label>No Rekening</label>
                                <input type="text" autocomplete="off" value="<?= $row->no_rek ?>" disabled readonly class="input w-full border mt-2 flex-1" placeholder="Masukan No Rekening Pemilik">
                                <input type="hidden" value="<?= $row->no_rek ?>" name="no_rekening" class="input w-full border mt-2 flex-1" placeholder="Masukan No Rekening Pemilik">
                            </div>
                            <div class="col-span-12 sm:col-span-12">
                                <label>Nama Bank</label>
                                <input type="text" autocomplete="off" onkeypress="return HurufNoEnter(event)" value="<?= $row->nama_bank ?>" name="nama_bank" class="input w-full border mt-2 flex-1" placeholder="Masukan Bank">
                            </div>

                            <div class="col-span-12 sm:col-span-12">
                                <label>Atas Nama</label>
                                <input type="text" autocomplete="off" onkeypress="return HurufNoEnter(event)" value="<?= $row->atas_nama ?>" name="atas_nama" class="input w-full border mt-2 flex-1" placeholder="Masukan Atas Nama Pemilik Rekening">
                            </div>
